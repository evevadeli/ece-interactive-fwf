MODULE sbcmod
   ...
   USE sbcfwf           ! surface boundary condition: additional freshwater forcing (runoffs+calving)
...
CONTAINS
   ...
   SUBROUTINE sbc_init
      ...
      NAMELIST/namsbc/ nn_fsbc   , ln_ana    , ln_flx, ln_blk_clio, ln_blk_core, ln_mixcpl,   &
         ...
         &             nn_lsm    , nn_limflx , nn_components, ln_cpl, ln_fwf     
      INTEGER  ::   ios
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc in configuration namelist', lwp )
      ...
      IF(lwp) THEN               ! Control print
         ...
         WRITE(numout,*) '              additional freshwater forcing files        ln_fwf      = ', ln_fwf
...
!     sas model time step has to be declared in OASIS (mandatory) -> nn_fsbc has to be modified accordingly
      ...
      !
                               CALL sbc_rnf_init               ! Runoff initialisation
      !
      IF( ln_fwf           )   CALL sbc_fwf_init               ! add additional fresh water fluxes (forced runoff + calving)
      !
   ...
   SUBROUTINE sbc( kt )
      ...
      CALL sbc_fwf_output   ! save rnf and calving with or without calling sbc_fwf
