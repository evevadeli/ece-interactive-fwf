MODULE sbccpl
   ...
   !!
   !!            3.3  ! 2022-04  (A. Juling) make compatible with sbcfwf.F90
   !!----------------------------------------------------------------------
   !!----------------------------------------------------------------------
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      IF( TRIM( sn_rcv_rnf%cldes ) == 'coupled' ) THEN
         ...
         l_rnfcpl              = .TRUE.                      ! -> if TRUE no need to read runoffs in sbcrnf
...
!!       &             +   zsprecip(:,:)                   * zsnw             * zqprec_ice(:,:) * r1_rhosn ! solid precip over ice
      ...
      ! IF( iom_use('hflx_rnf_cea') )     CALL iom_put('hflx_rnf_cea' , rnf(:,:) * zcptn(:,:) )
      ! some more outputs
      ! IF( iom_use('hflxs_snow_cea') )   CALL iom_put('hflxs_snow_cea'  ,   sprecip(:,:) * ( zcptn(:,:) )                              ) ! heat flux from snow (cell average)
