...
from config import ism, bm, fwf_distribution, depth #, FWF_total_yearmin #running_mean_period, 
...
file_baseline_fwf = f'{path_input}/original/FWF_LRF_y1850.nc' #baseline FWF (piControl)

file_larmip_ocean_mask = f'{ini_data_dir}/masks/LARMIP_ocean_regions_ORCA1.nc' #levermann2020
file_basal_melt_mask = f'{path_input}/basal_melt_mask_LARMIP_ORCA1.nc' #lambert2023
#file_basal_melt_mask = f'{path_input}/basal_melt_mask_Levermann2020_LARMIP_ORCA1.nc' #levermann2020
file_calving_mask = f'{path_input}/calving_mask_LARMIP_ORCA1.nc'#lambert2023
...
file_bm_depth1 = f'{run_dir}/basal_melt_depth1.nc' #shallowest depth #200 m lambert2023
file_bm_depth2 =f'{run_dir}/basal_melt_depth2.nc' # deepest depth #700 m lambert2023
...
for sector in sectors:
    ...
    depth_bnds_sector = dvp.sel_depth_bnds(depth) 
...
for sector in (sectors+['sum']):
    df_FWF_calving_source[sector] = bm_calv_distribution.loc[sector,'calv']/bm_calv_distribution.loc[sector,'sum']*df_dFWF.loc[year,sector]
    df_FWF_basal_melt[sector] = bm_calv_distribution.loc[sector,'bm']/bm_calv_distribution.loc[sector,'sum']*df_dFWF.loc[year,sector]
...
ds_FWF_baseline = xr.open_dataset(file_baseline_fwf)
FWF_basal_melt = FWF_basal_melt+ds_FWF_baseline.sorunoff_f.values
FWF_calving = FWF_calving+ds_FWF_baseline.socalving_f.values
